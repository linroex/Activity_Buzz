<?php
    ...
    $insert_sth = $db->prepare('insert into activity (name,description,created_at,url,date) values (?,?,?,?,?)');
    ...
    do{
        ...
        foreach ($activitys as $activity) {
            ...
            if(count($check_exist_sth->fetchAll()) == 0){
                ...
                var_dump($activity);
                $insert_sth->execute(array($activity['name'],$activity['desc'],date('Y:m:d',time()),$activity['url'],$activity['date']));
            }else{
                die();
            }
        }
